    ...
    size_t nb_char, double *cost_fun, size_t which_input)
{
    ...
    Matrix *tempo_b = d_tanh_matrix(neurons_z[nb_layers-1]); //sigmoid
...
void ocr_nn_train_loaded(Matrix **inputs, size_t nbinputs, Matrix **desired_outputs, neuralnet *nn)
{
    ...
    //if changing number of layers, do not forget to 
    //change it in init_neural_net
    //int layers[10] = {676, 200, 200, 150, 100, 100, 100, 100, 100, 26};
    //int layers[9] = {676, 300, 200, 200, 150, 100, 100, 100, 26};
    //int layers[8] = {676, 300, 200, 200, 150, 100, 100, 26};

    //initializing nn (generating weights, biases, neurons)
    //neuralnet *nn = init_neural_net(9, layers);
    ...
    *cost_fun /= nb_epochs*0.5;
    ...
    //free time
    // free_matrices(deltaB, nn->nb_layers-1);
...
void ocr_nn_train(Matrix **inputs, size_t nbinputs, Matrix **desired_outputs)
{
    ...
    *cost_fun = 0;
    ...
    for (size_t epoch=0; epoch < nb_epochs; epoch++)
    {
        ...
        eta = 0.05 * exp(-((double) epoch)/50000);
        printf("eta : %f\n", eta);
    ...
    *cost_fun /= nb_epochs*0.5;
    ...
    Matrix **loaded_biases = load_matrix_list("savefile/biases.bin", &i);

    //updates neural network with loaded values
    free(nn->weights);
    free(nn->biases);
    ...
    for (size_t t=0; t < nb_char; t++)
    {
        ...
        feed_forward(nn->neurons_z, nn->neurons_a, 
            nn->weights, nn->biases, nn->nb_layers);
        print_letter(inputs[t]);
        printf("\n--> [After loading] Detected letter: %c\n\n", 
            get_detected_letter(nn->neurons_a[nn->nb_layers - 1], nb_char));
    }

    //user input
    nn->neurons_z[0] = test_matrix;
    nn->neurons_a[0] = test_matrix;
    feed_forward(nn->neurons_z, nn->neurons_a, 
        nn->weights, nn->biases, nn->nb_layers);
    printf("-\nWith user inputs :\n");
    print_letter(test_matrix);
    printf("\n--> [After loading] Detected letter: %c\n\n", 
        get_detected_letter(nn->neurons_a[nn->nb_layers - 1], nb_char));
